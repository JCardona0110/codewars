...
class Song {
	constructor(title, artist) {
		Object.assign(this, { title, artist });
	}
	howMany(arr) {
		let count = arr.toUpperCase().filter(function (item, index) {
			arr.length();
		});
	}
}
...
class Song {
	...
	howMany(people) {
		let count = this.listener.size;
		people.forEach((p) => this.listener.add(p.toLowerCase()));
		return this.listener.size - count;
	}
}
